//encoding=GBK
#define _WIN32_WINNT 0x0600 //调用vista以上的系统api
#define ACE_NAME      L"ACE-Guard Client.exe"
#define SCRIPT_NAME   L"ACEDisappear.cmd"
#define SCANNING_TIME 5*1000


#include <windows.h>
#include <tlhelp32.h>//进程快照相关的头文件
#include <shlwapi.h> //拼接地址所用的头文件
#include <stdio.h>
/*新增：托盘所需*/
#include <shellapi.h>

/*---------- 新增：托盘消息与菜单ID ----------*/
#define WM_TRAY      (WM_USER + 1)
#define ID_TRAY_EXIT 2001

/*---------- 新增：全局变量 ----------*/
NOTIFYICONDATAW g_nid = {0}; //托盘图标结构
HMENU           g_hMenu = NULL;//右键菜单句柄
HANDLE          g_hThread = NULL;//监控线程句柄
BOOL            g_bRun = TRUE;  //线程退出标志

/*---------- 新增：函数前向声明 ----------*/
LRESULT CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);
void CreateTrayIcon(HWND);
void RemoveTrayIcon(void);
void ShowTrayMenu(HWND);
DWORD WINAPI MonitorThread(LPVOID); //监控线程入口

/*---------- 新增：监控线程(原main死循环移到这里) ----------*/
DWORD WINAPI MonitorThread(LPVOID)
{
    //得到脚本的路径(保持你的注释)
    wchar_t self[MAX_PATH];//存放结果的缓存
    GetModuleFileNameW(NULL,self, MAX_PATH);//NULL表示当前模块，self表示输出缓冲区，缓冲区大小
    PathRemoveFileSpecW(self);//把最后的文件名砍掉，得到当前的文件目录
    const wchar_t* fileName = SCRIPT_NAME;
    wchar_t fullPath[MAX_PATH];
    PathCombineW(fullPath, self , fileName);//拼接后的路径fullPath

    const wchar_t *TargetExe = ACE_NAME;//定义监视进程
    const wchar_t *OpenFile = fullPath;//定义脚本进程
    //为了使用相对路径，需要拼接软件自身地址和待执行文件的地址
    printf("\nScript Location:\n");
    wprintf(fullPath);

    while (g_bRun)
    {
        //创建进程快照
        HANDLE hSnap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);
        //第二个参数0表示不指定单个进程
        if(hSnap == INVALID_HANDLE_VALUE){
            //创建快照失败之后
            printf("CreateToolhelp32Snapshot failed");
            continue; //下次重试
        }
        PROCESSENTRY32W pe;//进程信息结构体
        pe.dwSize  = sizeof(pe);
        if(!Process32FirstW(hSnap,&pe)){
            //拿不到第一个进程的时候
            printf("Process32FirstW failed. Cloud not get the first Process.");
            CloseHandle(hSnap);
            continue; //下次重试
        }

        //遍历进程决定是否启动脚本
        do{
            if(_wcsicmp(pe.szExeFile,  TargetExe) == 0){
                //第二个参数使用runas表示以管理员身份运行脚本，open表示正常打开脚本
                ShellExecuteW(NULL, L"runas", OpenFile, NULL, self, SW_SHOWNORMAL);
                printf("\nDetected The Process ACE-Guard Client.exe\n");
                break;
            }
            //szExeFile是进程名，类型是宽字符，用%ls打印
        }while(Process32NextW(hSnap,&pe));

        CloseHandle(hSnap);
        Sleep(SCANNING_TIME); //保持你原来的10秒间隔
    }
    return 0;
}

/*新增：窗口过程，专门处理托盘与菜单消息*/
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    switch (msg)
    {
    case WM_CREATE:
        CreateTrayIcon(hWnd);//创建图标
        //启动监控线程(原main逻辑)
        g_hThread = CreateThread(NULL, 0, MonitorThread, NULL, 0, NULL);
        return 0;

    case WM_TRAY: //托盘回调
        if (lp == WM_RBUTTONUP) //右键弹菜单
        {
            SetForegroundWindow(hWnd);//避免菜单消失
            ShowTrayMenu(hWnd);
        }
        return 0;

    case WM_COMMAND:
        if (LOWORD(wp) == ID_TRAY_EXIT)
        {
            g_bRun = FALSE;        //通知线程退出
            if (g_hThread) WaitForSingleObject(g_hThread, 3000);
            PostQuitMessage(0);    //结束消息循环
            return 0;
        }
        break;

    case WM_DESTROY:
    case WM_CLOSE:
        RemoveTrayIcon(); //清理图标
        PostQuitMessage(0);
        return 0;
    }
    return DefWindowProcW(hWnd, msg, wp, lp);
}

/*新增：托盘小工具——创建图标*/
void CreateTrayIcon(HWND hWnd)
{
    g_nid.cbSize  = sizeof(g_nid);
    g_nid.hWnd   = hWnd;
    g_nid.uID    = 1234;
    g_nid.uFlags = NIF_ICON | NIF_MESSAGE | NIF_TIP;
    g_nid.uCallbackMessage = WM_TRAY;
    g_nid.hIcon  = (HICON)LoadImageW(NULL, IDI_INFORMATION, IMAGE_ICON, 0, 0, LR_SHARED);
    /* 替换 CreateTrayIcon 中的 wcscpy_s 调用 */
    wcscpy_s(g_nid.szTip,  ARRAYSIZE(g_nid.szTip),  L"ACE-Guard Watching");
    Shell_NotifyIconW(NIM_ADD, &g_nid);
}

/*新增：删除图标*/
void RemoveTrayIcon(void)
{
    Shell_NotifyIconW(NIM_DELETE, &g_nid);
}

/*新增：显示右键菜单*/
void ShowTrayMenu(HWND hWnd)
{
    if (!g_hMenu)
    {
        g_hMenu = CreatePopupMenu();
        AppendMenuW(g_hMenu, MF_STRING, ID_TRAY_EXIT, L"Exit");
    }
    POINT pt;
    GetCursorPos(&pt);
    TrackPopupMenu(g_hMenu, TPM_RIGHTBUTTON, pt.x, pt.y, 0, hWnd, NULL);
}

/*把main改成WinMain，用隐藏窗口承载托盘与消息循环*/
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, int)
{
    // 1. 注册窗口类
    const wchar_t CLASS_NAME[] = L"ACE_TrayCls";
    WNDCLASSEXW wc = {0};
    wc.cbSize         = sizeof(wc);
    wc.lpfnWndProc    = WndProc;
    wc.hInstance     = hInst;
    wc.lpszClassName  = CLASS_NAME;
    RegisterClassExW(&wc);

    // 2. 创建消息-only窗口（不可见）
    HWND hWnd = CreateWindowExW(0, CLASS_NAME, L"ACE_TrayWnd", 0,
                                0, 0, 0, 0, HWND_MESSAGE, NULL, hInst, NULL);

    // 3. 消息循环
    MSG msg;
    while (GetMessageW(&msg, NULL, 0, 0))
    {
        TranslateMessage(&msg);
        DispatchMessageW(&msg);
    }
    return 0;
}